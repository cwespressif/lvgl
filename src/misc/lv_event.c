...
void lv_event_desc_set_external_data(lv_event_dsc_t * dsc, void * ext_data, void (* destructor)(void * ext_data))
{
    LV_ASSERT_NULL(dsc);
...
bool lv_event_remove_dsc(lv_event_list_t * list, lv_event_dsc_t * dsc)
{
    ...
    for(uint32_t i = 0; i < size; i++) {
        ...
        if(event == dsc) {
#if LV_EXTERNAL_DATA_AND_DESTRUCTOR
            if (dsc->ext_data && dsc->destructor) dsc->destructor(dsc->ext_data);
#endif
...
void lv_event_remove_all(lv_event_list_t * list)
{
    ...
    for(uint32_t i = 0; i < size; i++) {
        ...
        if (dsc && dsc->ext_data && dsc->destructor) dsc->destructor(dsc->ext_data);
#endif
